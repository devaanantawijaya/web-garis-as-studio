...
export const PortfolioPage: React.FC<PortfolioPageProps> = ({
    ...
    const [selectedCategory, setSelectedCategory] = useState<Category>('KARYA PILIHAN');
    ...
    return (
        <div id="portfolio-page" className="pt-28 pb-24 bg-white text-neutral-900 min-h-screen">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="mb-10 text-center max-w-3xl mx-auto space-y-3">
                    ...
                    <p className="text-sm sm:text-base text-neutral-500 leading-relaxed">
                        Kumpulan karya arsitektur tropis, vila mewah, apartemen, hotel butik, dan desain interior eksklusif yang dirancang dengan presisi estetika dan fungsionalitas di Bali.
                    </p>
                </div>
                ...
                <div className="mb-8 flex flex-col md:flex-row items-center justify-between gap-4 bg-neutral-50 p-4 rounded-xs border border-neutral-200/80">
                    ...
                    <div className="text-xs font-semibold text-neutral-500">
                        Menampilkan <span className="font-bold text-neutral-900">{filteredProjects.length}</span> dari {projects.length} Proyek
                    </div>
                </div>
                    ...
                    {CATEGORIES.map((cat) => {
                        ...
                        return (
                            <button
                                ...
                                className={`relative pb-2.5 text-xs sm:text-sm font-bold uppercase tracking-wider whitespace-nowrap transition-colors cursor-pointer ${isActive ? 'text-neutral-950' : 'text-neutral-500 hover:text-neutral-800'
                                    }`}
                            >
                                {cat}
                                {isActive && (
                ...
                {filteredProjects.length === 0 ? (
                    <div className="py-20 text-center space-y-4">
                        <p className="text-neutral-400 text-base">Tidak ada proyek yang sesuai dengan kriteria pencarian.</p>
                        <button
                            onClick={() => {
                                setSelectedCategory('KARYA PILIHAN');
                                setSearchQuery('');
                            }}
                            className="px-5 py-2.5 bg-neutral-900 text-white text-xs font-bold uppercase tracking-wider rounded-xs"
                        >
                            Reset Filter
                        </button>
                    </div>
                        ...
                        <AnimatePresence mode="popLayout">
                            {filteredProjects.map((project, idx) => (
                                <motion.div
                                    ...
                                    className="group cursor-pointer bg-white border border-neutral-200/80 rounded-xs overflow-hidden shadow-xs hover:shadow-md transition-all flex flex-col"
                                >
                                    ...
                                    <div className="p-5 flex-1 flex flex-col justify-between space-y-3">
                                        ...
                                        <div className="pt-3 border-t border-neutral-100 flex items-center justify-between text-xs text-neutral-600 font-medium">
                                            ...
                                            <span className="font-bold text-neutral-950 uppercase tracking-wider group-hover:underline">
                                                Detail Proyek →
                                            </span>
                                        </div>
                                    </div>
